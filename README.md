...
For creative exploration, this repository also includes **The AI Artist**, a
variant focused on generative art. See `launch_artist.py` for usage.
...
### Run AI Artist Experiments

The `launch_artist.py` script follows the same interface but is geared toward
creative art projects. For example:

```bash
python launch_artist.py --model "gpt-4o-2024-05-13" --num-ideas 2
```
...
**Is there an AI Artist version?**

Yes. The script `launch_artist.py` mirrors the core pipeline but defaults to the
`sketch_rnn` template for generative drawing tasks. You can use it as a starting
point for creative experiments.
